---
layout: page
title: "Privacy Policy"
permalink: /privacy-policy/
description: "Privacy Policy for Ur-Lawyer explaining how we collect, use, and protect visitor information. Learn how personal data is handled on our legal information website."
---

This Privacy Policy describes how [**Ur-Lawyer**](https://ur-lawyer.github.io/) collects, uses, and protects the personal information of visitors. By using this website, you agree to the terms outlined below.

We aim to provide helpful legal information about various areas such as **personal injury lawyers, accident lawyers, divorce lawyers, immigration lawyers, family lawyers, criminal lawyers, medical malpractice lawyers, employment lawyers, child custody lawyers, tax lawyers, probate lawyers, traffic ticket lawyers, tenant lawyers, truck accident lawyers, car accident lawyers, and slip and fall lawyers** in the United States.  
However, we **do not request or store personal legal case details**.

---

## Information We Collect

We may collect **non-personal information**, such as:

- Browser type
- Device type
- Operating system
- Approximate geographic location
- Pages visited and time spent on the website
- Referring website links

This data is collected **anonymously** through web analytics tools like **Google Analytics** or similar platforms.

We **do not** collect:
- Social security numbers  
- Legal documents  
- Personal case history  
- Attorney-client communications  

---

## Cookies

Our website may use **cookies** to improve user experience. Cookies help:
- Load pages faster
- Save browser preferences
- Improve website performance and design

You may disable cookies through your browser settings at any time.

---

## Use of Information

The information we collect is used to:

- Improve website performance and usability
- Understand visitor interests
- Publish more relevant content
- Maintain website security

We **do not sell, trade, or share** user data with third parties for advertising or marketing.

---

## Third-Party Links

Our website may include links to:
- Legal resources
- Law firm websites
- Government information pages

These external websites have **their own privacy policies**.  
We are **not responsible** for their content or data handling practices.

---

## No Legal Advice or Attorney-Client Relationship

This website is for **informational purposes only** and does not replace professional legal advice.  
Using this site **does not create** an attorney-client relationship.

---

## Data Protection

We follow standard digital security practices, including:
- Secure hosting
- HTTPS encryption
- Routine threat monitoring

However, no website transmission is 100% secure. Users share information at their own discretion.

---

## Children’s Privacy

This website is **not intended for children under 13 years of age**.  
We do not knowingly collect data from minors.

---

## Updates to This Policy

We may update this Privacy Policy occasionally.  
Changes will be reflected on this page with the updated date below.

**Last Updated:** {{ site.time | date: "%B %d, %Y" }}

---

## Contact Us

If you have any questions regarding this Privacy Policy, please contact us through our **[Contact Page](https://ur-lawyer.github.io/contact/)**